...
export async function GET() {
  ...
  if (user?.role === "agency") {
    ...
    let agenciess = (await agencyColl.find({ type: "agency" }).toArray()).map(
      (d) => {
        delete d.createdAt;
        delete d.updatedAt;
        return {
    ...
    if (currentAgencyId) {
      currentagency = agenciess.find((a) => a._id === currentagency) || null;
    }

    if (!currentagency && agenciess.length > 0) {
      currentagency = agenciess[0];
    }
