...
export async function GET() {
  ...
  const items = await websiteCategoryService.list("asda" as string, websiteId);
...
export async function POST(req: Request) {
  ...
  const jar = await cookies();
  const websiteId = jar.get('current_website_id')?.value;
  const created = await websiteCategoryService.create("asda" as string, parsed.data, websiteId);
