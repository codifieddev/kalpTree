...
export async function GET(req: Request) {
  ...
  const items = await productVariantService.list("asda" as string, productId, { skip, limit, websiteId });
...
export async function POST(req: Request) {
  ...
  const jar = await cookies();
  const websiteId = jar.get('current_website_id')?.value;
  const created = await productVariantService.create("asda" as string, parsed.data, websiteId);
