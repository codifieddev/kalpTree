...
export class ProductVariantService {
  ...
  async list(tenantId: string | ObjectId, productId?: string, opts?: { skip?: number; limit?: number }) {
  ...
  async create(tenantId: string | ObjectId, data: Omit<ProductVariant, '_id' | 'tenantId' | 'createdAt' | 'updatedAt'>): Promise<ProductVariant> {
    const col = await this.getCollection();
    const tid = typeof tenantId === 'string' ? new ObjectId(tenantId) : tenantId;
    const doc: Omit<ProductVariant, '_id'> = { ...data, tenantId: tid as unknown as ObjectId, createdAt: new Date(), updatedAt: new Date() } as Omit<ProductVariant, '_id'>;
